...
public class Book : MonoBehaviour {
    ...
    public Image Left; // 넘기는 페이지 앞
    public Image LeftNext;
    public Image Right; // 넘기는 페이지 뒤
    public Image RightNext; // 넘기는 페이지 다음 페이지
    public UnityEvent OnFlip;
    float radius1, radius2; // 페이지 넓이, 페이지 대각선
    //Spine Bottom
    Vector3 sb;
    //Spine Top
    Vector3 st;
    ...
    public AutoFlip auto;

    void Start()
    {
        if (!canvas) canvas=GetComponentInParent<Canvas>();
        if (!auto) auto = GetComponent<AutoFlip>();
        if (!canvas) Debug.LogError("Book should be a child to canvas");

        Left.gameObject.SetActive(false);
        Right.gameObject.SetActive(false);
        UpdateSprites(); // 현재 상황 기준으로 오른쪽, 왼쪽 페이지 업데이트
        CalcCurlCriticalPoints(); // 각종 변수 설정

        float pageWidth = BookPanel.rect.width / 2.0f;
        float pageHeight = BookPanel.rect.height;
        NextPageClip.rectTransform.sizeDelta = new Vector2(pageWidth, pageHeight + pageHeight * 2); // 다음 페이지 보여주는 구간


        ClippingPlane.rectTransform.sizeDelta = new Vector2(pageWidth * 2 + pageHeight, pageHeight + pageHeight * 2); // 접히는 페이지 보여주는 구간
    ...
    {
        f = Vector3.Lerp(f, transformPoint(Input.mousePosition), Time.deltaTime * 10); // 마우스의 좌표를 f에 대입
    ...
    private float CalcClipAngle(Vector3 c,Vector3 bookCorner,out  Vector3 t1) // out-> 보통은 return으로 값을 가져오지만 두 개 이상의 값을 가져오기 위해서는 out을 이용한다.
    {
    ...
    private Vector3 Calc_C_Position(Vector3 followLocation)
    {
        ...
        // radius1-> 페이지의 모서리가 최대로 이동할 수 있는 반경을 이용해서 만든 원의 반지름

        float F_SB_distance = Vector2.Distance(f, sb);
        if (F_SB_distance < radius1) // 마우스가 범위를 벗어낫을 때 일정 범위 안에서만 페이지가 움직일 수 있게 만드는...?
            c = f;
        else
            c = r1;
        ...
        // radius2-> 페이지가 최대한 접힐 수 있는 범위를 이용해서 만든 원의 반지름
